...
"""
Circuit Function Template tests.

The entrypoint under ``source_files/`` imports its ``options`` sub-package and
``qiskit_serverless`` at module level. To keep the unit tests hermetic (no live
serverless cluster, no credentials) this package initializer:

1. puts ``source_files/`` on ``sys.path`` so the entrypoint's bare imports
   (``from circuit_function_entrypoint import ...``, ``from options import ...``)
   resolve exactly as they do when the artifact runs on a cluster, and
2. registers a stub ``qiskit_serverless`` package so ``get_logger``,
   ``update_status``, ``Job`` and friends resolve at import time without a
   running gateway.

The stub is installed before the test modules import the entrypoint, so it is in
place regardless of test-discovery order.
"""
